...
public class WorkFlowDefectController {
	@Resource
	...
	private static final Logger log = LoggerFactory
			.getLogger(WorkFlowDefectController.class);
	...
	@ResponseBody
	public Response<WorkFlowDefectDto> saveDefect(
		...
		if (null != defectDto && null != obj) {
			...
			defect.setDefectCode(UUIDUtils.getUUID());
			String taskKey = UUIDUtils.getUUID(); // 生成任务key
			String procKey = UUIDUtils.getUUID(); // 生产处理进程key
			defect.setProcId(procKey);
			defect.setProcState(WorkFlowConstant.PROC_STATUS_WAITING);
			defect.setDealResult(WorkFlowConstant.TASK_DEAL_RESULT_NOT_START);
			defect.setEnterpriseId(user.getEnterpriseId());
			defect.setCreateTime(System.currentTimeMillis());
			defect.setStartTime(System.currentTimeMillis());
			defect.setCreateUserId(user.getId());
			defect.setAlarmNum(defect.getAlarmIds().split(",").length+"");
			int result = workFlowDefectService.insertWorkFlowDefectM(defect);
			if (1 == result) {
				//更新告警的状态和时间
				Map<String,Object> condition = new HashMap<>();
				condition.put("ids", defect.getAlarmIds());
				condition.put("alarmType", defect.getAlarmType());
				condition.put("confirmTime", System.currentTimeMillis());
				condition.put("statusId", "3");
				alarmService.updateAlarm(condition);
				// 2. 创建任务关联关系
				WorkFlowTaskUserM taskUser = createWorkFlowTaskUserM(defectDto,
						taskKey, procKey, user);
				workFlowTaskUserService.insertWorkFlowTaskUserM(taskUser); // 插入任务人员关联关系

				// 3.创建处理任务
				WorkFlowTaskM task = createWorkFlowTaskM(defectDto, taskKey,
						procKey, WorkFlowConstant.PROC_STATUS_WAITING,
						WorkFlowConstant.TASK_NOT_OPERATION_STATUS, user);
				task.setFileId(null); //删除前端传递过来的file_id
				workFlowTaskService.insertWorkFlowTaskM(task); // 插入审核任务

				// 4. 创建处理进度
				WorkFlowProcessM process = new WorkFlowProcessM(); // 创建当前缺陷处理进度
				process.setProcId(procKey);
				process.setProcState(WorkFlowConstant.PROC_STATUS_WAITING);
				process.setStartTime(System.currentTimeMillis());
				// process.setEndTime(defectDto.getEndTime());
				process.setCurrentTaskId(taskKey); // 当前任务id
				process.setCurrentTaskAssignee(defectDto.getCurrentUserName());// 当前处理人
				process.setCreateUser(user.getLoginName());
				process.setStationCode(defectDto.getStationCode());
				process.setEnterpriseId(user.getEnterpriseId());
				workFlowProcessService.insertWorkFlowProcess(process);// 插入处理进程

				response.setCode(ResponseConstants.CODE_SUCCESS);
				response.setMessage(ResponseConstants.CODE_SUCCESS_VALUE);
				log.info("insert defect success, device id is "
						+ defectDto.getDevId());
			} else {
				response.setCode(ResponseConstants.CODE_FAILED);
				response.setMessage(ResponseConstants.CODE_FAILED_VALUE);
				log.error("insert defect fail, device id is "
						+ defectDto.getDevId());
			}
		} else {
			response.setCode(ResponseConstants.CODE_FAILED);
			response.setMessage(ResponseConstants.CODE_FAILED_VALUE);
			log.error("insert defect fail, defect data is null or no user login ");
		}
	...
	@RequestMapping(value = "/alarmToDefect", method = RequestMethod.POST)
	@ResponseBody
	public Response<AlarmToDefectVO> alarmToDefect(
			@RequestBody AlarmToDefectVO vo) {
		Response<AlarmToDefectVO> response = new Response<>();
		if (null != vo && null != vo.getAlarmIds()) {
			List<AlarmToDefectVO> list = null;
			
			if (null != vo.getType() && vo.getType().equals("1") && vo.getAlarmIds() != null) { //
				list = workFlowDefectService.getAlarmMByAlarmMId(vo.getAlarmIds());
			} else if (null != vo.getType() && vo.getType().equals("2")) {
				list = workFlowDefectService.getAnalysisByIds(vo.getAlarmIds());
			}
			if (null != list && list.size() > 0) {
				response.setCode(ResponseConstants.CODE_SUCCESS);
				response.setMessage(ResponseConstants.CODE_SUCCESS_VALUE);
				AlarmToDefectVO result = new AlarmToDefectVO();
				BeanUtils.copyProperties(list.get(0), result);
				result.setType(vo.getType());
				result.setAlarmIds(list.get(0).getAlarmId()+"");
				result.setAlarmNames(list.get(0).getAlarmName());
				for (int i = 1; i < list.size(); i++) {
					result.setAlarmIds(result.getAlarmIds() + "," + list.get(i).getAlarmId());
					result.setAlarmNames(result.getAlarmNames() + "," + list.get(i).getAlarmName());
				}
				response.setResults(result);
				
				log.info("alarm to defect success, alarm id is  "
						+ vo.getAlarmIds());
			} else {
				response.setCode(ResponseConstants.CODE_FAILED);
				response.setMessage(ResponseConstants.CODE_FAILED_VALUE);
				log.error("alarm to defect fail, alarm id is  "
						+ vo.getAlarmIds());
			}
		} else {
			response.setCode(ResponseConstants.CODE_FAILED);
			response.setMessage(ResponseConstants.CODE_FAILED_VALUE);
			log.error("alarm to defect fail, alarm id is null");
		}
		return response;
	}
	 ...
	 * 统计当前用户下有多少个电站有缺陷
	 */
	@RequestMapping(value = "/getAllWorkFlowDefectMCount", method = RequestMethod.POST)
	@ResponseBody
	public Response<Integer> getAllWorkFlowDefectMCount(
			@RequestBody WorkFlowDefectDto defectDto) {
		Response<Integer> response = new Response<>();
		if (null != defectDto && null != defectDto.getUserId()) {
			Integer result = workFlowDefectService
					.selectAllWorkFlowDefectMCount(defectDto);
			if (null != result) {
				response.setResults(result);
				response.setCode(ResponseConstants.CODE_SUCCESS);
				response.setMessage(ResponseConstants.CODE_SUCCESS_VALUE);
				log.info("count defect by userId success, userId is "
						+ defectDto.getUserId());
			} else {
				response.setCode(ResponseConstants.CODE_FAILED);
				response.setMessage(ResponseConstants.CODE_FAILED_VALUE);
				log.error("count defect by userId fail, userId is "
						+ defectDto.getUserId());
			}
		} else {
			response.setCode(ResponseConstants.CODE_FAILED);
			response.setMessage(ResponseConstants.CODE_FAILED_VALUE);
			log.error("count defect by userId fail, userId is null");
		}
		return response;
	}
	 ...
	 * 查询当前用户下的所有的电站名称
	 */
	@RequestMapping(value = "/getStationsByUserId", method = RequestMethod.POST)
	@ResponseBody
	public Response<List<StationInfoM>> getStationsByUserId(
			@RequestBody WorkFlowDefectDto defectDto) {
		Response<List<StationInfoM>> response = new Response<>();
		if (null != defectDto && null != defectDto.getUserId()) {
			List<StationInfoM> result = workFlowDefectService
					.selectStationsByUserId(defectDto.getUserId());
			if (null != result) {
				response.setCode(ResponseConstants.CODE_SUCCESS);
				response.setMessage(ResponseConstants.CODE_SUCCESS_VALUE);
				response.setResults(result);
				log.info("get station by userId success, userId is " + defectDto.getUserId());
			} else {
				response.setCode(ResponseConstants.CODE_FAILED);
				response.setMessage(ResponseConstants.CODE_FAILED_VALUE);
				log.error("get station by userId fail, userId is "
						+ defectDto.getUserId());
			}
		} else {
			response.setCode(ResponseConstants.CODE_FAILED);
			response.setMessage(ResponseConstants.CODE_FAILED_VALUE);
			log.error("get station by userId fail, userId is null");
		}
		return response;
	}

	/**
	 * 查询当前电站下的所有的设备的model_version_code和名称和设备类型的名称
	 */
	@RequestMapping(value = "/getDeviceInfoByStationCode", method = RequestMethod.POST)
	@ResponseBody
	public Response<List<DeviceInfo>> getDeviceInfoByStationCode(
			@RequestBody WorkFlowDefectDto defectDto) {
		Response<List<DeviceInfo>> response = new Response<>();
		if (null != defectDto && null != defectDto.getStationCode()) {
			List<DeviceInfo> result = workFlowDefectService
					.selectDeviceInfoByStationCode(defectDto.getStationCode());
			if (null != result) {
				response.setResults(result);
				response.setCode(ResponseConstants.CODE_SUCCESS);
				response.setMessage(ResponseConstants.CODE_SUCCESS_VALUE);
				log.info("get deviceInfo by stationCode success, stationCode is "
						+ defectDto.getStationCode());
			} else {
				response.setCode(ResponseConstants.CODE_FAILED);
				response.setMessage(ResponseConstants.CODE_FAILED_VALUE);
				log.error("get deviceInfo by stationCode fail, stationCode is "
						+ defectDto.getStationCode());
			}
		} else {
			response.setCode(ResponseConstants.CODE_FAILED);
			response.setMessage(ResponseConstants.CODE_FAILED_VALUE);
			log.error("get deviceInfo by stationCode success, stationCode is null");
		}
		return response;
	}
	 ...
	 * 统计个人任务总条数
	 */
	@RequestMapping(value = "/getWorkFlowTaskCountByUserId", method = RequestMethod.POST)
	@ResponseBody
	public Response<Integer> getWorkFlowTaskCountByUserId(
			@RequestBody WorkFlowDefectDto defectDto, HttpSession session) {
		Response<Integer> response = new Response<>();
		if (null == defectDto.getUserId()) {
			Object obj = session.getAttribute("user");
		...
		if (null != defectDto && null != defectDto.getUserId()
				&& null != defectDto.getTaskState()) {
			QueryWorkFlowDefect condition = new QueryWorkFlowDefect();
			condition.setDefect(defectDto);
			Integer result = workFlowDefectService
					.selectWorkFlowTaskCountByUserId(condition);
			if (null != result) {
				response.setResults(result);
				response.setCode(ResponseConstants.CODE_SUCCESS);
				response.setMessage(ResponseConstants.CODE_SUCCESS_VALUE);
				log.info("count task by userId success, userId is "
						+ defectDto.getUserId());
			} else {
				response.setCode(ResponseConstants.CODE_FAILED);
				response.setMessage(ResponseConstants.CODE_FAILED_VALUE);
				log.error("count task by userId fail, userId is "
						+ defectDto.getUserId());
			}
		} else {
			response.setCode(ResponseConstants.CODE_FAILED);
	...
	@ResponseBody
	public Response<List<AlarmToDefectVO>> getDefectAlarm(@RequestBody WorkFlowDefectDto defectDto, HttpServletRequest request)
	{
		...
		{
			response.setCode(ResponseConstants.CODE_FAILED);
			response.setMessage(ResourceBundleUtil.getBizKey(request.getHeader(ResponseConstants.REQ_LANG), "workFlowDefectController.search.alarm")); // "查询关联告警的id为空或告警类型为空"
			log.error("count task by userId fail, userId is null or condition data is null");
		}
		return response;
	}
	 ...
	 * 查询用户电站的数据
	 */
	@RequestMapping(value = "/getStationUser", method = RequestMethod.POST)
	@ResponseBody
	public Response<List<Map<String,Object>>> getStationUser(HttpSession session, HttpServletRequest request)
	{
		...
		{
			UserInfo user = (UserInfo)obj;
			List<Map<String,Object>> list = workFlowDefectService.getStationUser(user.getId(),user.getType_());
		...
		{
			response.setCode(ResponseConstants.CODE_FAILED);
			response.setMessage(ResourceBundleUtil.getBizKey(request.getHeader(ResponseConstants.REQ_LANG), "workFlowDefectController.users.login")); // "用户登陆失效或用户未登录"
			log.error("query user station data fail, user not login");
		}
		return response;
	}
}
