		...
		h = {
			...
			transformTozTreeFormat: function(b, a) {
				var c, d, f = b.data.simpleData.idKey,
					...
					e = b.data.key.children,
					p = b.data.simpleData.isParentKey || "isParent";
				if (!f || f == "" || !a) return [];
				if (j.isArray(a)) {
					var h = [],
						i = [];
					...
					for (c = 0, d = a.length; c < d; c++) 
						i[a[c][g]] && a[c][f] != a[c][g] ? (i[a[c][g]][e] || (i[a[c][g]][e] = []), i[a[c][g]][p] = true,i[a[c][g]][e].push(a[c])) : h.push(a[c]);
					return h
				} else return [a]
			}
		},
		...
		i = {
			...
			cancelPreSelectedNode: function(b,
				...
				for (var c = h.getRoot(b).curSelectedList, d = c.length - 1; d >= 0; d--)
					if (!a || a === c[d]){
						k(c[d], e.id.A, b).parent().removeClass(e.node.CURSELECTEDP);
			...
			selectNode: function(b, a, c) {
				...
				k(a, e.id.A, b).parent().addClass(e.node.CURSELECTEDP);
