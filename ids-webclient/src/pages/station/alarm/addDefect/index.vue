<template>
  ...
  <el-dialog
    ...
    <SelectAlarmDialog v-bind:devId="devId" v-if="isShowChooseAlarm" @choose-alarm="chooseAlarms"
                       :is-show-choose-alarm.sync="isShowChooseAlarm"></SelectAlarmDialog>
  </el-dialog>
</template>

<script type="text/ecmascript-6">
  import SelectAlarmDialog from '@/pages/station/alarm/chooseAlarmDialog/index.vue'
    ...
    methods: {
      ...
      chooseAlarms (chooseRows, devId, stationCode) { // 选择告警的事件
        ...
        for (let i = 0; i < len; i++) {
          ...
          alarmIds.push(tempAlarm.alarmId)
      ...
      getDevById (devId) { // 查询设备信息
        let self = this
        // 查询设备的相关信息
        OperationService.getDevById(devId).then(resp => {
          var datas = (resp.code === 1 && resp.results) || {};
      ...
      setFormAttrs (datas) {
        ...
        this.$set(this.formData, 'stationAddress', datas.stationAddr || '')
        this.$set(this.formData, 'devName', datas.devName || '')
        this.$set(this.formData, 'devId', datas.id || null)
        this.$set(this.formData, 'devType', datas.devType || '')
        this.$set(this.formData, 'devVersion', datas.devVersion || '')
      },
