...
export default {
  data() {
    return {
      ...
      assetsInputDialogVisible: false, // 设备窗口
      distributeForm: {
        title: "分发代理商", // 分发代理商
        agent: "", // 代理商
        installer: "", // 安装商
      ...
      assetsDistributeDialogVisible: false, // 分发窗口
      elementSize: "",
      data: [{
        label: '全部',
        id: "1",
  ...
  methods: {
    ...
    // 分发代理商
    distribute() {
      this.$data.assetsDistributeDialogVisible = true;
    },
    ...
    setData() {
      let self = this;
      epcService.getManufacturerTree({'type': 'epc'}).then(resp => {
        self.$data.data = resp.results;
      });
      epcService.getAssetsOfManufacturer({'type': 'epc'}).then(resp => {
