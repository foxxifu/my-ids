...
export default {
  data() {
    return {
      ...
      exchangeForm: {
        titleType: "1",
        showAgreementBtn: true, // 是否显示同意按钮
        showRejectionBtn: true, // 是否显示拒绝按钮
  ...
  methods: {
    ...
    viewRow(index, tableData) {
      ...
      this.$data.exchangeForm.showRejectionBtn = true;
      ...
      if (this.$data.exchangeTableData.length === 12) {
    ...
    approvedViewRow(index, tableData) {
      ...
      this.$data.isShowMain = false;
      // this.$data.isShowExchangeDialog = true;
      this.$data.exchangeForm.showAgreementBtn = false;
      this.$data.exchangeForm.showRejectionBtn = false;
      ...
      if (this.$data.exchangeTableData.length === 10) {
    ...
    arraySpanMethod({row, column, rowIndex, columnIndex}) {
      if (rowIndex === 7 || rowIndex === 8 || rowIndex === 10) {
        if (columnIndex === 1) {
          return [1, 3];
        }
      }
    ...
    setData() {
      let self = this;
      epcService.getExchangeOfGoodsTree({'type': 'manufacturer'}).then(resp => {
        self.$data.data = resp.results;
      });
      epcService.getExchangeOfGoodsOfManufacturer({'type': 'manufacturer', 'status': "applying"}).then(resp => {
        self.$data.applyingTableData = resp.results;
      });
      epcService.getExchangeOfGoodsOfManufacturer({'type': 'manufacturer', 'status': "approved"}).then(resp => {
        self.$data.approvedTableData = resp.results;
      });
      epcService.getExchangeOfGoodsOfManufacturer({'type': 'manufacturer', 'status': "refused"}).then(resp => {
