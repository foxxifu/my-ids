    ...
    <el-dialog :title="$t('devMan.devAcc')" :visible="isShowDialog" width="35%" :before-close="handleClose">
      <el-form ref="addDevForm" :model="formData" label-width="170px" :rules="rules" class="demo-ruleForm">
       ...
       <!-- <el-form-item label="设备关联数采" prop="parentId" :rules="[{required:formData.devTypeId !== 2, message: '设备关联数采必选'}]" >
        ...
        <el-form-item :label="$t('devMan.numberSN')" prop="parentSn" :rules="[{required:true, message: $t('devMan.numberSNReq')}]" >
          <el-select v-model="formData.parentSn" :placeholder="$t('devMan.choose')" style="width: 200px;" filterable>
            <el-option v-for="(item, index) in parentSnCodeList" :key="index" :label="item" :value="item"></el-option>
          </el-select>
          <el-input v-model="inputParentSn" :placeholder="$t('devMan.addNumSN')" maxlength="20" style="width: 255px;" @blur="inputBlur"></el-input>
        </el-form-item>
  ...
  export default {
    ...
    methods: {
      queryDevVersionInfo () { // 查询设备版本信息,每次进入的时候查询, 查询到设备的版本和协议和设备类型等信息
        ...
        devAccessService.getMqqtVersionList({protocal: 'MQTT'}).then(resp => { // 获取协议的版本信息
      ...
      submitForm () { // 表单提交的信息
        ...
        this.$refs.addDevForm.validate((valid) => {
          if (valid) {
            ...
            devAccessService.insertMqqtDevInfo(self.formData).then(resp => {
              if (resp.code === 1) {
                self.$message('添加成功')
                // 2.通知后台去处理数据，因为不知道当前的协议是什么，就在所有的设备改变，都通知mqqt清除缓存
      ...
      getMqqtParentSnList() { // 获取mqqt父节点的sn号
        devAccessService.getMqqtParentSnList({stationCode: this.stationCode}).then(resp => {
          let datas = (resp.code === 1 && resp.results) || []
          this.orgParentSnCodeList = datas;
          this.parentSnCodeList = [].concat(datas);
        })
      },
      inputBlur() { // 失去焦点就给当前的选择的对象复制
        if (this.inputParentSn) { // 如果存在
          this.$set(this.formData, 'parentSn', this.inputParentSn)
        } else {
          let sn = this.formData.parentSn
          if (sn && this.parentSnCodeList.filter(item => item === sn).length === 0) { // 如果当前列表中没有对应的sn号就需要设置当前选中的为空
            this.$set(this.formData, 'parentSn', '')
          }
        }
      }
    ...
    watch: {
      inputParentSn(newValue, oldValue){
        if (!newValue) { // 如果没有新的value
          this.parentSnCodeList = [].concat(this.orgParentSnCodeList);
        } else {
          let tempArr = this.orgParentSnCodeList.filter(item => newValue === item)
          if (tempArr.length === 0) { // 不存在就去修改最后一个
            if (this.orgParentSnCodeList.length === this.parentSnCodeList.length) { // 长度一样就新增
              this.parentSnCodeList.push(newValue)
            } else { // 否则就修改最后一个
              this.parentSnCodeList[this.parentSnCodeList.length - 1] = newValue;
            }
          } else {
            if (this.orgParentSnCodeList.length < this.parentSnCodeList.length &&
              this.parentSnCodeList[this.parentSnCodeList.length - 1] !== newValue) {
              this.parentSnCodeList.pop(); // 删除最后一个
            }
          }
        }
      }
