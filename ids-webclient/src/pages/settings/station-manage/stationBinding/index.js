...
export default {
  ...
  methods: {
    ...
    setStationTables(params, callback) {
      let self = this;
      stationInfo.getNoBindingStationInfoByPage(params).then(resp => {
        if (resp.code === 1) {
    ...
    setStationInfoByParam(record){
      if (record) {
        ...
        this.$data.stationForm.name = record.domainName ? record.domainName + "" : '';
        ...
        let areaCodeDetails = (record.areaCode && record.areaCode.split("@")) || "";
        if (areaCodeDetails) {
