...
import OneTicketDialog from '../operationOneTicket/oneTicketDialog.vue'
...
export default {
  ...
  created () {
    ...
    this.processStatus = [
      {label: this.$t('io.workTick.processStatus.createFirstWork'), value: 'createFirstWork'},
      {label: this.$t('io.workTick.processStatus.issueFirstWork'), value: 'issueFirstWork'},
      {label: this.$t('io.workTick.processStatus.chargeFirstSubmit'), value: 'chargeFirstSubmit'},
      {label: this.$t('io.workTick.processStatus.recipientFirstSure'), value: 'recipientFirstSure'},
      {label: this.$t('io.workTick.processStatus.licensorSure'), value: 'licensorSure'},
      {label: this.$t('io.workTick.processStatus.chargeApplyStop'), value: 'chargeApplyStop'},
      {label: this.$t('io.workTick.processStatus.licensorStop'), value: 'licensorStop'},
      {label: this.$t('io.workTick.processStatus.licensorWorkStop'), value: 'licensorWorkStop'},
      {label: this.$t('io.workTick.processStatus.processEnd'), value: 'processEnd'},
    ]
  },
  mounted: function () {
    this.$nextTick(() => {
  ...
  methods: {
    ...
    addTicket () {
      ...
      this.ticketCreateDialog.type = 'create'
    ...
    modifyTicket () {
      this.isDetail = false
      let len = this.ticketTableData.selection && this.ticketTableData.selection.length
      if (len === 1) {
        let ticket = this.ticketTableData.selection[0]
        if (ticket.processStatus === 'processEnd' || ticket.processStatus === 'giveup') {
          this.$message((ticket.processStatus === 'processEnd' && this.$t('io.workTick.noModifyEnd')) || this.$t('io.workTick.noModifyZf'))
          return
        }
        ...
        this.$msgbox({message: len === 0 ? this.$t('selectOneRecord') : this.$t('io.chooseRecord').replace('{0}', len), title: this.$t('confirm')}) // `请选择一条数据修改,当前选择的记录数为:${len}条`
      }
    },
    // 查询一条工作票数据
    searchOneTick (stationCode, definitionId) {
      this.$set(this, 'ticketCreateForm', {workTime: []})
      this.allUsers = []
      let self = this
    ...
    exportTicket () {
      if (this.ticketTableData.selection && this.ticketTableData.selection.length === 1) {
        ...
        this.$confirm(this.$t('io.workTick.sureExport')).then(() => {
          window.open('/biz/workTicket/export?definitionId=' + ticket.definitionId + '&time=' + Date.parseTime() + '&lang=' + lang, '_self')
        })
      } else {
        this.$msgbox({message: this.$t('selectOneRecord'), title: this.$t('confirm')})
      }
    ...
    printTicket () {
      if (this.ticketTableData.selection && this.ticketTableData.selection.length === 1) {
        let ticket = this.ticketTableData.selection[0]
        this.$confirm(this.$t('io.workTick.surePrint')).then(() => {
          window.open('/biz/workTicket/export?definitionId=' + ticket.definitionId + '&time=' + Date.parseTime(), '_self')
        })
      } else {
        this.$msgbox({message: this.$t('selectOneRecord'), title: this.$t('confirm')})
      }
    ...
    processHandler (row) {
      ...
      */
      ...
      // 弹出框标题
      this.operatTransDatas.operatTitle = (processStatus === 'createFirstWork' && this.$t('io.workTick.submit')) || this.$t('io.workTick.sh')
    ...
    // 工作流作废
    endWorkFlow (row) {
      this.$confirm(this.$t('io.workTick.sureZf')).then(() => {
