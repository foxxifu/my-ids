<template>
  <div class="smart-alarm">
    ...
    <div>
      <div style="float: left;">
        <el-form :inline="true" :model="searchData" class="demo-form-inline">
          <el-form-item :label="$t('io.alarm.plantName')">
            <el-input v-model="searchData.stationName" :label="$t('io.alarm.plantName')" clearable></el-input>
          </el-form-item>
          <el-form-item :label="$t('io.alarm.devName')">
            <el-input v-model="searchData.devAlias" :label="$t('io.alarm.devName')" clearable></el-input>
          </el-form-item>
          <el-form-item :label="$t('io.alarm.alarmLevel')">
            <el-select v-model="searchData.alarmLevel" :label="$t('io.alarm.alarmLevel')" clearable>
          ...
          <el-form-item :label="$t('io.alarmStatus')">
            <el-select v-model="searchData.alarmState" :placeholder="$t('io.alarmStatus')" clearable>
              <!-- 未确认-->
      ...
      <div style="float: right">
        <!-- 清除按钮 -->
        <el-button @click="clearOrSureAlarm('clear')" type="primary">{{$t('io.alarm.clear')}}</el-button>
        <!-- 确认按钮 -->
        <el-button @click="clearOrSureAlarm('sure')" type="primary">{{$t('io.alarm.ack')}}</el-button>
        <!-- 转工单按钮 -->
        <el-button @click="clickToDefect" type="primary">{{$t('io.alarm.createOrder')}}</el-button>
      </div>
    </div>
    <div class="list-bar">
      <!-- 表格数据 -->
      <el-table :data="list" :maxHeight="maxHeight" border width="100%" ref="smartAlarmTable" :fit="true" row-key="id"
        ...
        <el-table-column :label="$t('io.alarmName')" align="center">
    ...
    <el-dialog
      ...
      append-to-body>
      <AlarmDetailDialgContent v-if="isShowDetailDialog"
                               :needSearch="[true, {id:rowData.id}, 2, {stationName: rowData.stationName}]"
